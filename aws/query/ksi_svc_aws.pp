    ...
    select
      ...
      case
        when el.arn is not null
          then l.name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then l.name || ' is exempt.'
    ...
    select
      ...
      case
        when ec.arn is not null
          then c.domain_name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then c.domain_name || ' is exempt.'
    ...
    select
      ...
      case
        when ea.arn is not null
          then 'Listener has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
    ...
    select
      ...
      case
        when ee.arn is not null
          then ec.replication_group_id || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then ec.replication_group_id || ' is exempt.'
...
query "ksi_svc_04_aws_check" {
  sql = <<-EOQ
    ...
    select
      ...
      case
        when es.arn is not null
          then s.stack_name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then s.stack_name || ' is exempt.'
    ...
    select
      ...
      case
        when er.arn is not null
          then 'Config rule ' || r.name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then 'Config rule ' || r.name || ' is exempt.'
  ...
  sql = <<-EOQ
    ...
    select
      ...
      case
        ...
        when e.arn is not null
          then r.repository_name || ' is exempt.'
...
query "ksi_svc_08_aws_check" {
  sql = <<-EOQ
    ...
    select
      ...
      case
        when ee.arn is not null
          then 'Elastic IP ' || coalesce(eip.public_ip, eip.allocation_id) || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then 'Elastic IP ' || coalesce(eip.public_ip, eip.allocation_id) || ' is exempt.'
    ...
    select
      ...
      case
        when ea.arn is not null
          then ami.name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then ami.name || ' is exempt.'
    ...
    select
      ...
      case
        when es.arn is not null
          then sg.group_name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then sg.group_name || ' is exempt.'
    ...
    select
      ...
      case
        when em.arn is not null
          then m.virtual_service_name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').'
        when e.arn is not null
          then m.virtual_service_name || ' is exempt.'
