          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-01' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-01.1' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        ...
        select
          ...
          case
            when ee.arn is not null
              then u.name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || '). MFA required.' || coalesce(' Reason: ' || e.exemption_reason || '.', '')
            when e.arn is not null
              then u.name || ' is exempt from MFA requirement.' || coalesce(' Reason: ' || e.exemption_reason || '.', '')
  ...
  sql = <<-EOQ
    ...
    -- Check for users with passwords over 90 days old
        select
          ...
          case
            when ee.arn is not null
              then u.name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').' || coalesce(' Reason: ' || e.exemption_reason || '.', '')
            when e.arn is not null
              then u.name || ' is exempt.' || coalesce(' Reason: ' || e.exemption_reason || '.', '')
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-03' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-03.1' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_roles as (
        ...
        select
          ...
          case
            when eu.arn is not null
              then k.user_name || ' has EXPIRED exemption (expired: ' || e.exemption_expiry || ').' || coalesce(' Reason: ' || e.exemption_reason || '.', '')
            when e.arn is not null
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-05' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-05.1' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_policies as (
  ...
  sql = <<-EOQ
        with exempt_users as (
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-05' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-05.2' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_users as (
          select arn from exempt_users
          where exemption_expiry is not null and exemption_expiry::date < current_date
  ...
  sql = <<-EOQ
        with exempt_roles as (
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-05' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-05.3' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_roles as (
          select arn from exempt_roles
          where exemption_expiry is not null and exemption_expiry::date < current_date
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-07' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-07.1' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_users as (
          select arn from exempt_users
          where exemption_expiry is not null and exemption_expiry::date < current_date
  ...
  sql = <<-EOQ
        with exempt_users as (
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-07' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-07.2' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_users as (
          select arn from exempt_users
          where exemption_expiry is not null and exemption_expiry::date < current_date
