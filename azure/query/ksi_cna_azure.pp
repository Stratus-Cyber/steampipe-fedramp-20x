...
query "ksi_cna_01_2_azure_check" {
  sql = <<-EOQ
        with exempt_2 as (
          select
            id as exempt_id,
        ...
        select
          ...
          case
            when exp_2.exempt_id is not null
              then vm.name || ' has EXPIRED exemption (expired: ' || e_2.exemption_expiry || ').' || coalesce(' Reason: ' || e_2.exemption_reason || '.', '')
            when e_2.exempt_id is not null
              then vm.name || ' is exempt.' || coalesce(' Reason: ' || e_2.exemption_reason || '.', '')
...
query "ksi_cna_01_3_azure_check" {
  sql = <<-EOQ
...
query "ksi_cna_01_4_azure_check" {
  sql = <<-EOQ
        with exempt_3 as (
          select
            id as exempt_id,
...
query "ksi_cna_02_1_azure_check" {
  sql = <<-EOQ
        with exempt_1 as (
          select
            id as exempt_id,
    ...
    -- Check VM disks are encrypted (CIS Azure 7.1)
        select
          ...
          case
            when exp_1.exempt_id is not null
              then vm.name || ' has EXPIRED exemption (expired: ' || e_1.exemption_expiry || ').' || coalesce(' Reason: ' || e_1.exemption_reason || '.', '')
            when e_1.exempt_id is not null
              then vm.name || ' is exempt.' || coalesce(' Reason: ' || e_1.exemption_reason || '.', '')
...
query "ksi_cna_02_2_azure_check" {
  sql = <<-EOQ
        with exempt_2 as (
          select
            id as exempt_id,
...
query "ksi_cna_02_3_azure_check" {
  sql = <<-EOQ
        with exempt_3 as (
          select
            id as exempt_id,
...
query "ksi_cna_02_4_azure_check" {
  sql = <<-EOQ
        with exempt_4 as (
          select
            id as exempt_id,
...
query "ksi_cna_02_5_azure_check" {
  sql = <<-EOQ
        with exempt_5 as (
          select
            id as exempt_id,
        ...
        select
          ...
          case
            when exp_5.exempt_id is not null
              then name || ' has EXPIRED exemption (expired: ' || e_5.exemption_expiry || ').' || coalesce(' Reason: ' || e_5.exemption_reason || '.', '')
            when e_5.exempt_id is not null
              then name || ' is exempt.' || coalesce(' Reason: ' || e_5.exemption_reason || '.', '')
...
query "ksi_cna_03_1_azure_check" {
  sql = <<-EOQ
        with exempt_1 as (
          select
            id as exempt_id,
...
query "ksi_cna_03_2_azure_check" {
  sql = <<-EOQ
        with exempt_2 as (
          select
            id as exempt_id,
...
query "ksi_cna_03_3_azure_check" {
  sql = <<-EOQ
        with exempt_3 as (
          select
            id as exempt_id,
...
query "ksi_cna_03_4_azure_check" {
  sql = <<-EOQ
        with exempt_4 as (
          select
            id as exempt_id,
...
query "ksi_cna_03_5_azure_check" {
  sql = <<-EOQ
        with exempt_5 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-CNA-03' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-CNA-03.5' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_5 as (
          select exempt_id from exempt_5
          where exemption_expiry is not null and exemption_expiry::date < current_date
...
query "ksi_cna_04_1_azure_check" {
  sql = <<-EOQ
        with exempt_1 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-CNA-04' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-CNA-04.1' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_1 as (
          select exempt_id from exempt_1
          where exemption_expiry is not null and exemption_expiry::date < current_date
...
query "ksi_cna_04_2_azure_check" {
  sql = <<-EOQ
        with exempt_2 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-CNA-04' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-CNA-04.2' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_2 as (
          select exempt_id from exempt_2
          where exemption_expiry is not null and exemption_expiry::date < current_date
...
query "ksi_cna_04_3_azure_check" {
  sql = <<-EOQ
        with exempt_3 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-CNA-04' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-CNA-04.3' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_3 as (
          select exempt_id from exempt_3
          where exemption_expiry is not null and exemption_expiry::date < current_date
...
query "ksi_cna_04_4_azure_check" {
  sql = <<-EOQ
        with exempt_4 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-CNA-04' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-CNA-04.4' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_4 as (
          select exempt_id from exempt_4
          where exemption_expiry is not null and exemption_expiry::date < current_date
...
query "ksi_cna_04_5_azure_check" {
  sql = <<-EOQ
        with exempt_5 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-CNA-04' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-CNA-04.5' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_5 as (
          select exempt_id from exempt_5
          where exemption_expiry is not null and exemption_expiry::date < current_date
