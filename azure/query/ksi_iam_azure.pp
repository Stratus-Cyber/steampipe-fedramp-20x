...
# Updated for Turbot Pipes workspace schema (azure.* and azuread.*)

query "ksi_iam_01_1_azure_check" {
  sql = <<-EOQ
...
query "ksi_iam_01_2_azure_check" {
  sql = <<-EOQ
...
query "ksi_iam_02_1_azure_check" {
  sql = <<-EOQ
        with exempt_1 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-02' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-02.1' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_1 as (
...
query "ksi_iam_02_2_azure_check" {
  sql = <<-EOQ
        with exempt_2 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-02' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-02.2' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_2 as (
    ...
    -- Check Key Vault secret expiration dates set (CIS Azure 8.2)
        ...
        from
          azure.azure_key_vault_secret
          left join exempt_2 as e_2 on azure.azure_key_vault_secret.id = e_2.exempt_id
          left join expired_2 as exp_2 on azure.azure_key_vault_secret.id = exp_2.exempt_id
  EOQ
}

query "ksi_iam_02_3_azure_check" {
  sql = <<-EOQ
        with exempt_3 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-02' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-02.3' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_3 as (
...
query "ksi_iam_03_1_azure_check" {
  sql = <<-EOQ
...
query "ksi_iam_03_2_azure_check" {
  sql = <<-EOQ
        with exempt_1 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-03' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-03.2' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_1 as (
          select exempt_id from exempt_1
          where exemption_expiry is not null and exemption_expiry::date < current_date
    ...
    -- Check managed identities used for Azure resources (best practice)
        ...
        from
          azure.azure_compute_virtual_machine
          left join exempt_1 as e_1 on azure.azure_compute_virtual_machine.id = e_1.exempt_id
          left join expired_1 as exp_1 on azure.azure_compute_virtual_machine.id = exp_1.exempt_id
  EOQ
}

query "ksi_iam_03_3_azure_check" {
  sql = <<-EOQ
        with exempt_2 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-03' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-03.3' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_2 as (
          select exempt_id from exempt_2
          where exemption_expiry is not null and exemption_expiry::date < current_date
    ...
    -- Check SQL Server uses Azure AD authentication (CIS Azure 4.1.1)
        ...
        from
          azure.azure_sql_server
          left join exempt_2 as e_2 on azure.azure_sql_server.id = e_2.exempt_id
          left join expired_2 as exp_2 on azure.azure_sql_server.id = exp_2.exempt_id
  EOQ
}

query "ksi_iam_03_4_azure_check" {
  sql = <<-EOQ
        with exempt_3 as (
          select
            id as exempt_id,
          ...
          where
            tags->>'${var.exemption_tag_key}' is not null
              and ('KSI-IAM-03' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':'))
                OR 'KSI-IAM-03.4' = any(string_to_array(tags->>'${var.exemption_tag_key}', ':')))
        ),
        expired_3 as (
          select exempt_id from exempt_3
          where exemption_expiry is not null and exemption_expiry::date < current_date
    ...
    -- Check App Service uses managed identity (CIS Azure 9.1)
        ...
        from
          azure.azure_app_service_web_app
          left join exempt_3 as e_3 on azure.azure_app_service_web_app.id = e_3.exempt_id
          left join expired_3 as exp_3 on azure.azure_app_service_web_app.id = exp_3.exempt_id
  EOQ
}

query "ksi_iam_05_1_azure_check" {
  sql = <<-EOQ
...
query "ksi_iam_05_2_azure_check" {
  sql = <<-EOQ
    -- Check for overly permissive role assignments at subscription level
        select
          ...
          ra.subscription_id
        from
          azure.azure_role_assignment as ra
          join azure.azure_role_definition as rd on ra.role_definition_id = rd.id
  EOQ
}

query "ksi_iam_05_3_azure_check" {
  sql = <<-EOQ
...
query "ksi_iam_05_4_azure_check" {
  sql = <<-EOQ
...
query "ksi_iam_06_1_azure_check" {
  sql = <<-EOQ
...
query "ksi_iam_06_2_azure_check" {
  sql = <<-EOQ
    -- Check security alerts are configured
        select
          ...
          subscription_id
        from
          azure.azure_security_center_contact
  EOQ
}

query "ksi_iam_07_1_azure_check" {
  sql = <<-EOQ
...
query "ksi_iam_07_2_azure_check" {
  sql = <<-EOQ
